...
class MoveTest implements Context {
    ...
    private static final String CMD_NAME = MoveTest.class.getSimpleName();
    ...
    void run(final Case testCase) {
        ...
        // when...
        Move.job(this, testCase.args.apply(mainPath))
            .run();
    ...
    enum Case {
        ...
        FLAT_Y_M_D(path -> List.of(CMD_NAME, path.toString(), "(moved)/@Y/@M/@D/@N.@X"),
                   "MoveTest.flat.Y.M.D.txt"),
        FLAT_Y_n_x(path -> List.of(CMD_NAME, path.toString(), "(moved)\\@Y\\@N\\@X\\@F"),
                   "MoveTest.flat.Y.n.x.txt"),
        DEEP_Y_M_D(path -> List.of(CMD_NAME, path.toString(), "../(moved)/@Y/@M\\@D/@N.@X", "-r"),
                   "MoveTest.deep.Y.M.D.txt"),
        DEEP_Y_M_D_IGNORE(path -> List.of(CMD_NAME, path.toString(), "..\\(moved)/@Y\\@M/@D\\@F", "-R", "api", "job"),
                          "MoveTest.deep.Y.M.D.ignore.txt"),
        DEEP_Y_M_D_HASH(path -> List.of(CMD_NAME, path.toString(), "(moved)/@Y/@M/@D/@#-pp.@X", "-r"),
                        "MoveTest.deep.Y.M.D.#.txt"),
        DEEP_R_HASH(path -> List.of(CMD_NAME, path.toString(), "@P/@N@#-qrs.@X", "-r"),
                    "MoveTest.deep.R.#.txt"),
        NOTHING_TO_DO(path -> List.of(CMD_NAME, path.toString(), "@P/@F", "-r"),
