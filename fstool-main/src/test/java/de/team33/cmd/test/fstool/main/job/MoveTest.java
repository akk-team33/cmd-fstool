...
class MoveTest implements Context {
    ...
    enum Case {
        ...
        FLAT_Y_M_D(path -> List.of(path.toString(),
                                     "(moved)/$Y/$M/$D/$N.$X"),
                   "MoveTest.flat.Y.M.D.txt"),
        FLAT_Y_n_x(path -> List.of(path.toString(),
                                     "(moved)/$Y/$N/$X/$F"),
                   "MoveTest.flat.Y.n.x.txt"),
        DEEP_Y_M_D(path -> List.of(path.toString(),
                                     "(moved)/$Y/$M/$D/$N.$X",
                                     "-r"),
                   "MoveTest.deep.Y.M.D.txt"),
        DEEP_Y_M_D_IGNORE(path -> List.of(path.toString(),
                                            "(moved)/$Y/$M/$D/$F",
                                            "-R", "api", "job"),
                          "MoveTest.flat.Y.M.D.txt"),
        DEEP_Y_M_D_HASH(path -> List.of(path.toString(),
                                          "(moved)/$Y/$M/$D/$#$X",
                                          "-r"),
                        "MoveTest.deep.Y.M.D.#.txt"),
        DEEP_R_HASH(path -> List.of(path.toString(),
                                      "$P/$N$#$X",
                                      "-r"),
                    "MoveTest.deep.R.#.txt"),
        NOTHING_TO_DO(path -> List.of(path.toString(),
                                        "$P/$F",
                                        "-r"),
                      "SomeFiles.initial.txt");
